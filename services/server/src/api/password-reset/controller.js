...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your neya-coinbase-server account.<br>
        ...
        &mdash; neya-coinbase-server Team
      `
      return sendMail({ toEmail: email, subject: 'neya-coinbase-server - Password Reset', content })
    })
