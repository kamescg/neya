...
export const useLoggedInEffect = (box, props = {}) => {
  ...
  useEffect(() => {
    const select = dot(box)
      .get(`auth.spaces.${props.space}.instance`)
      .value();
...
export const useStorageSelectorEffect = (box, props) => {
  ...
  useEffect(() => {
    const select = dot(box)
      .get(`@.${props.address}.spaces.${props.space}.${props.selector}`)
      .value();
...
export const useThreadJoinEffect = (box, props) => {
  ...
  useEffect(() => {
    const select = dot(box)
      .get(`auth.threads.${props.threadName}`)
      .value();
...
export const useThreadReadyEffect = (box, props) => {
  ...
  useEffect(() => {
    const select = dot(box)
      .get(`auth.threads.${props.threadName}.instance`)
      .value();
    if (!isReady && select) setIsReady(true);
  }, [box.auth.threads]);
