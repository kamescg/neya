    ...
    if (state.store.sets.length > 0) {
      ...
      switch (selected.type) {
        case 'SET_SINGLE_REQUEST':
          state.auth.instance[selected.access].set(
            selected.key,
            selected.value
          );
        ...
        case 'SET_MULTIPLE_REQUEST':
          state.auth.instance[selected.access].setMultiple(
            selected.keys,
            selected.inputs
          );
          dispatch({
          ...
          if (selected) {
            const runEffect = async () => {
              ...
              try {
                if (space) {
                  if (append) {
                    const data = await state.spaces[space].instance[access].get(
                      append
                    );
                    if (data) {
                      /**
                      ...
                      if (key) {
                        ...
                        listUpdated = Array.isArray(data)
                          ? [...data, inputs]
                          : [data, inputs];
                      }
                    } else {
                    ...
                    const list = await state.spaces[space].instance[access].set(
                      append,
                      listUpdated
                    );
                    dispatch({
                    ...
                    await state.spaces[space].instance[access].setMultiple(
                      keys,
                      inputs
                    );
                    dispatch({
        ...
        case 'SET_MERGE_REQUEST':
          ...
          try {
            if (selected.space) {
              ...
              state.auth.instance[selected.access]
                .set(selected.key, dataMarge)
                .then(res => {
                  dispatch({
                    type: 'SET_MERGE_SUCCESS',
